  ...
  methods: {
    async fetchPublicUserSongs() {
      try {
        ...
        console.error(
          "Error Fetching Stringify Exclusive Artists Songs: ",
          error
        );
    ...
    async fetchFeaturedArtists() {
      try {
        ...
        const response = await this.$plainAxios.get(
          "/api/v1/music/featured_artists"
        );
    ...
    async fetchFeaturedSongs(){
        try{
            this.loading.featuredSongs = true;
            const response = await this.$plainAxios.get('/api/v1/music/featured_songs');
            this.featuredSongs = response.data.songs || [];
        } catch(error) {
            console.error('Error Fetchinng Featured Songs: ', error);
            this.featuredSongs = this.getFallbackSongs();
        } finally {
            this.loading.featuredSongs = false;
        }
    },
      ...
      return [
        {
          ...
          genre: "Alternative",
          artwork:
            "https://is1-ssl.mzstatic.com/image/thumb/Music211/v4/92/9f/69/929f69f1-9977-3a44-d674-11f70c852d1b/24UMGIM36186.rgb.jpg/300x300bb.jpg",
        },
        {
          id: 2,
          name: "Kendrick Lamar",
          genre: "Hip-Hop",
          artwork:
            "https://is1-ssl.mzstatic.com/image/thumb/Music112/v4/9e/57/22/9e5722a9-0c83-58e1-4b3f-7f3d6d0b22a3/196589486069.jpg/300x300bb.jpg",
        },
        {
          id: 3,
          name: "Taylor Swift",
          genre: "Pop",
          artwork:
            "https://is1-ssl.mzstatic.com/image/thumb/Music112/v4/9c/91/83/9c918303-e0a4-30d0-6e51-9993a635b4e7/196589486069.jpg/300x300bb.jpg",
        },
        {
          id: 4,
          name: "The Weeknd",
          genre: "R&B",
          artwork:
            "https://is1-ssl.mzstatic.com/image/thumb/Music122/v4/b2/47/6b/b2476b5c-7620-602f-933e-6d8e3557e274/22UMGIM15338.rgb.jpg/300x300bb.jpg",
        },
      ];
    },
  ...
  <div class="min-h-screen bg-gray-800 py-8">
    <div class="max-w-7xl mx-auto px-4">
      ...
      <section class="mb-12">
        ...
        <div
          v-else-if="error"
          class="text-center py-8 bg-red-900/20 rounded-lg"
        >
        ...
        <!-- No Songs State -->
        <div
          v-else-if="!exclusiveSongs || exclusiveSongs.length === 0"
          class="text-center py-12 bg-gray-700 rounded-lg"
        >
        ...
        <div v-else class="flex space-x-6 overflow-x-auto pb-4">
          ...
          >
            ...
            <p class="text-blue-300 text-sm mb-2 truncate">
              {{ song.artist?.name || "Unknown Artist" }}
            </p>
            <p class="text-green-300 text-xs mb-3 truncate">
        ...
        <div v-if="loading.artists" class="flex space-x-6 overflow-x-auto py-4">
          <div v-for="n in 6" :key="n" class="flex-shrink-0 w-32 animate-pulse">
            <div class="text-center">
              <div
                class="bg-gray-700 rounded-full w-24 h-24 mx-auto mb-2"
              ></div>
        ...
        <div v-else class="flex space-x-8 overflow-x-auto pb-4">
          ...
          >
            <div class="relative mb-3">
              ...
              />
              <div
                class="absolute inset-0 rounded-full bg-black opacity-0 group-hover:opacity-20 transition-opacity"
              ></div>
            </div>
            <h3
              class="text-white font-semibold text-sm mb-1 truncate w-28 mx-auto"
            >
              {{ artist.name }}
            </h3>
            <p class="text-gray-400 text-xs truncate w-28 mx-auto">
              {{ artist.genre }}
            </p>
          </div>
        </div>
      ...
      <section class="mb-12">
        ...
        <div v-else class="flex space-x-6 overflow-x-auto pb-4">
          ...
          >
            ...
            <p class="text-blue-300 text-sm mb-2 truncate">{{ song.artist }}</p>
            <p class="text-gray-400 text-xs mb-3">
              {{ song.genre }} • {{ song.release_year }}
            </p>
